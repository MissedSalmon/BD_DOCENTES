...
def generate_docente_csv(filename, num_rows):
    ...
    with open(os.path.join(directory, filename), mode='w', newline='') as file:
        ...
        for _ in range(num_rows):
            legajo = fake.unique.random_int(min=1, max=99999999)
...
def generate_empresa_csv(filename, num_rows, uni_cuits):
    ...
    with open(os.path.join(directory, filename), mode='w', newline='') as file:
        ...
        for _ in range(num_rows):
            emp = fake.random_element(uni_cuits)
...
def generate_actividades_investigacion_csv(filename, institucion_cuits, num_rows):
    ...
    with open(os.path.join(directory, filename), mode='w', newline='') as file:
        ...
        for _ in range(num_rows):
            inv=fake.unique.random_int(min=1000, max=999999)
...
def generate_titulos_csv(filename, num_rows):
    ...
    with open(os.path.join(directory, filename), mode='w', newline='') as file:
        ...
        for _ in range(num_rows):
            Tit=fake.unique.random_int(min=1, max=100000)
            Titulo.append(Tit)
            writer.writerow([
                Tit,
                fake.word(),
...
docente_legajos = generate_docente_csv('Docente.csv', 50000)
contacto_numeros = generate_contactos_csv('Contactos.csv', 20000)
publicaciones_publi = generate_publicaciones_csv('Publicaciones.csv', 2000)
reuniones_reu = generate_reuniones_cientificas('Reuniones.csv', 2000)
generate_idiomas_csv('Idiomas.csv', 200)
...
generate_pasividad_obtenida_csv('PasividadObtenida.csv',docente_legajos, 200)
ubicaciones = generate_ubicacion_csv('Ubicaciones.csv', 2000)
...
empresa_cuits=generate_empresa_csv('Empresa.csv', 10000, institucion_cuits) 
tareas_no_oficiales=generate_tareas_no_oficiales_csv('tareasnooficiales.csv', institucion_cuits, 2000)
